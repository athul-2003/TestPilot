...
| 7 | Ship — README, Action, demo | `phase/7-ship` | 🟡 In progress |
...
**Steps**
- [x] `README.md`: positioning line verbatim, the problem, quickstart, the real Phase 6 metrics table (quoted from the committed `metrics-report.md`, not restated by hand), and an honest limitations section — including the two findings Phase 6 surfaced and chose not to fix.
- [x] **First real functionality added in this phase, not just docs:** `src/cli.ts` — the D1 `git diff <merge-base> HEAD` integration that every prior phase deferred. Real `git merge-base` + `git diff` subprocess calls, a clear error (not a silent wrong diff) when the checkout is too shallow to find a merge base, `--json`/`--out`/`--diff-file` flags, and an entry-point guard so importing its pure functions for testing doesn't trigger a real git diff and model call as a side effect.
- [x] `examples/testpilot-workflow.yml` — the copy-pasteable adopter-facing job, with `fetch-depth: 0` and the exact `pull-requests: write` + `issues: write` permissions spelled out. **Deliberately not at `.github/workflows/testpilot-example.yml`** as originally planned: any `.yml` file in that directory is auto-discovered and actually *run* by GitHub Actions, which would have made the example a real, self-referential, broken workflow on this repo's own PRs. Caught before committing it, not after.
- [x] **D6, both forms.** `action.yml` — a composite Action wrapping the CLI with no logic of its own, distributed via git tag (no npm publish required for the *Action* to work — `github.action_path` gives it this repo's own contents at the pinned ref). `package.json` reshaped for npm publishing (`bin`, `exports`, a `files` allowlist, `private: true` removed) — **not actually published**; that's a manual, deliberate step for the user, not something this session does autonomously.
- [x] Privacy posture — already stated plainly since Phase 0/4, re-verified word for word in this pass: diffs go to a third-party provider by default, "self-hostable" is not "local by default."
- [x] **D5 numbers published**, pulled from real measurements across Phases 3–7, not re-estimated: ~1,600–1,900 tokens and ~2.1–2.5s per run on Groq.
- [x] `CONTRIBUTING.md`, `CODE_OF_CONDUCT.md` (Contributor Covenant 2.1), issue templates (bug report, feature request — no Discussions link, since `gh api repos/athul-2003/TestPilot --jq '.has_discussions'` confirmed it isn't enabled and a dead link is worse than no link), PR template.
- [ ] **60–90s demo — not done, and not something this session can do.** Recording a screen capture requires an actual human at an actual screen; noted here rather than silently skipped.
- [ ] **`v0.1.0` tag — pending.** Tagging is a visible, external milestone marker; confirming with the developer before creating and pushing it, same as the earlier call not to `npm publish` autonomously.

**Exit gate:** ✅ Met — a clean clone, following only the README, gets a working run on a test repo. Verified for real: `git clone --branch phase/7-ship`, `npm install` (576 packages, cold), `cp .env.example .env` plus a real key, `node src/cli.ts --repo-root . --base main`. Full success: confidence 0.90, `src/cli.test.ts` correctly identified as **must-run** (rule 5 — it was the one file directly edited in that diff), the other 28 tests correctly classified `should-run` with the by-now-familiar cautious rationale. The first two attempts hit Groq's TPM rate limit — a genuine, external cap from this session's own heavy dogfooding volume, not a defect: both failures happened only on the model call itself, confirming every step before it (clone, install, `.env`, real `git merge-base`/`git diff` against the fresh clone's own `origin/main`, the full workflow up to the LLM request) was already correct.
...
| **D5** | Cost & latency budget per PR run | **Measured from Phase 3, published in the README from Phase 7.** ~1,600–1,900 tokens and ~2.1–2.5s per run on Groq, real numbers from real runs across Phases 3–7 — comfortably under the **$0.05/60s** target. Reported next to the minutes-saved claim, not separately, since a tool that sells CI savings has no business hiding its own cost. | Phase 3 → 7 |
| **D6** | Distribution: npm, GitHub Action, or both? | **Both, implemented.** `package.json` is publish-ready (`bin`/`exports`/`files`) but not published — that's a manual step. `action.yml` is a composite Action that distributes via git tag, not npm — GitHub checks out this repo's own contents to `github.action_path` for whatever ref a consumer pins, so the Action works today, before any npm publish happens. It's a wrapper only (`npm ci` + the CLI + a PR comment step) — no logic lives there, so the two forms can never drift. | Phase 7 |
