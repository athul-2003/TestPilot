...
_Model: `openai/gpt-5.4-mini`_

**Missed regressions: 0 of 6 scenarios.** This is the trust metric — it must read 0.

**Estimated CI-minute reduction:** 1.8 minutes across 6 scenarios (placeholder per-test-type assumptions — see the report footnote in each run; not a measurement of real test durations).

**Selection efficiency:** 18/20 of the hand-derived "safe to skip" tests were actually skipped — the gap between this and 100% is real tests run that a perfectly-informed tool would not have needed to.
